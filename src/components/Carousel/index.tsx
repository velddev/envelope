...
export const Carousel = ({ columns, children }: CarouselProps) => {
  ...
  const colCount = Array.isArray(columns) ? useBreakpointValue(columns) : columns
  ...
  return (
    <VStack spacing="0" w="full" overflow="hidden">
      <Box position="relative" w="full">
        ...
        <Box overflowX="hidden" pb="12" w="full">
          <MotionBox
            ...
            w="100%"
          >
            <SimpleGrid templateColumns={`repeat(${children.length}, ${90 / colCount}%)`} spacing="8">
              {children.map((x, i) => (
      ...
      <Stepper index={index} onIndexChange={setIndex} pageCount={1 + Math.max(0, children.length - colCount)} />
    </VStack>
  )
}
